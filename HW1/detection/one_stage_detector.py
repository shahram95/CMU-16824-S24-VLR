...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def forward(self, images: torch.Tensor):
        ...
        for key, value in features.items():
            backbone_feats[key] = value
        
        level_p5 = self.lateral_c5(backbone_feats["c5"])
        level_p4 = self.lateral_c4(backbone_feats["c4"]) + F.interpolate(level_p5, scale_factor=2, mode="nearest")
        level_p3 = self.lateral_c3(backbone_feats["c3"]) + F.interpolate(level_p4, scale_factor=2, mode="nearest")

        fpn_feats["p3"] = self.output_p3(level_p3)
        fpn_feats["p4"] = self.output_p4(level_p4)
        fpn_feats["p5"] = self.output_p5(level_p5)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        print(stem_channels)
        def create_conv_layers(channel_in, channel_out):
            return [
        ...
        # Apply the simplified function to create stem_cls and stem_box
        stem_cls = create_conv_layers(in_channels, stem_channels[0])
        stem_box = create_conv_layers(in_channels, stem_channels[1])
        ...
        def create_prediction_conv(in_channels, out_channels, kernel_size=3, padding=1):
            return nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=padding)

        # Replace these lines with your code, keep variable names unchanged.
        self.pred_cls = create_prediction_conv(in_channels, num_classes)
        self.pred_box = create_prediction_conv(in_channels, 4)
        self.pred_ctr = create_prediction_conv(in_channels, 1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        def predict_and_reshape(preds, feats, num_outputs):
            
            return preds.permute(0, 2, 3, 1).contiguous().view(feats.size(0), -1, num_outputs)
        ...
        for level_name, feats in feats_per_fpn_level.items():
            ...
            class_logits[level_name] = predict_and_reshape(self.pred_cls(cls_feats), feats, self.num_classes)
            boxreg_deltas[level_name] = predict_and_reshape(self.pred_box(box_feats), feats, 4)
            centerness_logits[level_name] = predict_and_reshape(self.pred_ctr(box_feats), feats, 1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for level in ["p3", "p4", "p5"]:
            shape_per_fpn_level[level] = torch.tensor(backbone_feats[level].shape).to(torch.device("cuda"))
        ...
        def match_gt_boxes_to_locations(gt_box, locations_per_level, stride_per_level):
            matched_boxes = fcos_match_locations_to_gt(locations_per_level, stride_per_level, gt_box)
            return matched_boxes

        def compute_deltas_and_centerness(matched_boxes, locations_per_level, stride_per_level):
            deltas = {}
            centerness = {}
            for level, locations in locations_per_level.items():
                deltas[level] = fcos_get_deltas_from_locations(locations, matched_boxes[level], stride_per_level[level])
                centerness[level] = fcos_make_centerness_targets(deltas[level])
            return deltas, centerness

        gt_batch_count = gt_boxes.shape[0]
        matched_gt_boxes = [None] * gt_batch_count
        matched_gt_deltas = [None] * gt_batch_count
        matched_gt_ctr = [None] * gt_batch_count

        for i in range(gt_batch_count):
            gt_box = gt_boxes[i, :, :]
            matched_gt_boxes[i] = match_gt_boxes_to_locations(gt_box, locations_per_fpn_level, strides_per_fpn_level)
            matched_gt_deltas[i], matched_gt_ctr[i] = compute_deltas_and_centerness(matched_gt_boxes[i], locations_per_fpn_level, strides_per_fpn_level)
        ...
        device = matched_gt_boxes.device
        target_cls_logits = torch.zeros(batch_size, num_locations, 20, device=device)

        cls_indices = matched_gt_boxes[:,:,4].long()
        valid_mask = (cls_indices != -1)

        for b in range(batch_size):
            for loc in range(num_locations):
                if valid_mask[b, loc]:
                    cls_idx = cls_indices[b, loc]
                    target_cls_logits[b, loc, cls_idx] = 1

        loss_cls = sigmoid_focal_loss(pred_cls_logits, target_cls_logits, reduction='none')
        loss_box = 0.25 * F.l1_loss(pred_boxreg_deltas, matched_gt_deltas.to(device), reduction="none")
        valid_box_mask = (matched_gt_deltas >= 0)
        loss_box = loss_box.to(device) * valid_box_mask.to(device)

        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits, matched_gt_ctr.to(device).unsqueeze(-1), reduction="none")
        valid_ctr_mask = (matched_gt_ctr >= 0).to(device)
        loss_ctr = loss_ctr * valid_ctr_mask.unsqueeze(-1)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            most_confident_value, most_confident_cls = torch.max(level_pred_scores, dim=1)
            
            # Step 2:
            # Replace "pass" statement with your code
            level_pred_classes = most_confident_cls[most_confident_value > test_score_thresh]
            level_pred_scores, level_deltas, level_locations = [var[mask].cuda() for var in (most_confident_value, level_deltas, level_locations)]
            ...
            # Replace "pass" statement with your code
            level_pred_boxes[:, ::2].clamp_(min=0, max=torch.tensor([image.shape[3], image.shape[2]])) 

...
def ae_loss(model, x):
    ...
    batch_loss = nn.MSELoss(reduction='sum')(reconstructed_x, x)
...
def vae_loss(model, x, beta = 1):
    ...
    epsilon = torch.randn_like(std).to(x.device)
    reconstructed_x = model.decoder(mu + std * epsilon)
    recon_loss = F.mse_loss(reconstructed_x, x, reduction='sum') / x.size(0)

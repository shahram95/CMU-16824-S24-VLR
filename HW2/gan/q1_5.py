...
def compute_discriminator_loss(
    ...
    grads = torch.autograd.grad(outputs=discrim_interp, inputs=interp,  grad_outputs=grad_outputs, create_graph=True)[0]
    
    grad_norm = grads.norm(2, dim=(1, 2, 3))
    penalty = (grad_norm - 1)**2
    loss = (discrim_fake - discrim_real + lamb * penalty).mean()
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
    return loss

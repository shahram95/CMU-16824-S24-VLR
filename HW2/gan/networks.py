...
class UpSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        # 1. Repeat x channel-wise upscale_factor^2 times
        repeat_factor = int(self.upscale_factor**2)
...
class DownSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        channels_per_group = channels // int(self.downscale_ratio**2)
        x = x.view(batch, int(self.downscale_ratio**2), channels_per_group, height, width).permute(1, 0, 2, 3, 4)
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        # to the layer output.
        ##################################################################
        main_path = self.layers(x)
        residual_path = self.upsample_residual(x)
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        main_path = self.layers(x)
        downsampled_path = self.downsample_residual(x)
        return main_path + downsampled_path
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlock(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        assert x.size(1) == residual.size(1), "Mismatch in feature channels between input and residual output."
        return residual + x
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def forward_given_samples(self, z):
        ...
        x = self.dense(z)
        num_channels = 128    
        x = x.view(-1, num_channels, self.starting_image_size, self.starting_image_size)    
        x = self.layers(x)
        return x
        ##################################################################
        #                          END OF YOUR CODE                      #
...
class Discriminator(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        return self.dense(self.layers(x).sum((2, 3)))
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################

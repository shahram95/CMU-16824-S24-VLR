...
def get_optimizers_and_schedulers(gen, disc):
    ...
    # 100K iterations.
    ##################################################################
    scheduler_discriminator = torch.optim.lr_scheduler.LinearLR(optim_discriminator, 1, 0, 500e3)
    scheduler_generator = torch.optim.lr_scheduler.LinearLR(optim_generator, 1, 0, 100e3)
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
    return (
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                # discriminator on it.
                ###################################################################
                interp = torch.rand(len(train_batch), 1, 1, 1, device=train_batch.device) * (train_batch - fake_batch) + fake_batch
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=amp_enabled):
                        ...
                        samples_min = generated_samples.amin(dim=(1, 2, 3), keepdim=True)
                        samples_max = generated_samples.amax(dim=(1, 2, 3), keepdim=True)

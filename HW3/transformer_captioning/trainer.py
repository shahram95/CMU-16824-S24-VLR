...
class Trainer(object):
    ...
    def loss(self, predictions, labels):
        ...
        predictions_flat = predictions.contiguous().view(-1, predictions.size(-1))
        labels_flat = labels.contiguous().view(-1)
        loss = torch.nn.CrossEntropyLoss(ignore_index=self.model._null)(predictions_flat, labels_flat)
        return loss
